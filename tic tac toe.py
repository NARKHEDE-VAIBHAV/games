...
def ender(ids):  # for ending the game
    print(ids," WIN ")
    exitf = input("press enter to exit")
    exit()

def playnum1(box): # to cheake player is win or not
...
def playpc1(box): # to cheke computer is win or not
...
def player(box):  # player will start from here
  i = 1
  while (i < 11): 
    if (number == []):
         playnum1(box)
    ...
    if (player in base):
      if (player in em):
        ...
        if (number == []):
          print(box)
          print("no moves left")
        comp = random.choice(number) # computer input
        compnum.append(comp)
        em.append(comp)
        number.remove(comp)
...
def comp(box):  # computer will start form here
...
if ((start%2) == 0):  #startiing the game
  player(box)
else:
  comp(box)
